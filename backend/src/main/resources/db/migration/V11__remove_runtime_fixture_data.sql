...
where user_id in (
    ...
    where demo_user = true
       or email like 'e2e-%@time-table.test'
       or email like 'visual-%@time-table.test'
       or email in ('local@time-table.dev', 'demo@time-table.dev')
)
  and (source_type = 'DEFAULT_ROUTINE' or source_ref = 'default-routine');
...
where user_id in (
  ...
  and (
    (title = '영어 공부' and description = '이번 달 영어 공부 20시간 누적')
