...
Time Table은 주간 시간표, 할 일, 목표, 집중 실행 흐름을 하나의 화면 경험으로 묶어주는 실행형 생산성 워크스페이스입니다.

대부분의 생산성 도구는 일정은 캘린더에, 할 일은 태스크 앱에, 목표는 노트에 흩어져 있습니다. Time Table은 그 분산된 흐름을 다시 하나로 묶어, “이번 주를 어떻게 설계하고 오늘 무엇을 실행할지”를 더 선명하게 보여주는 것을 목표로 합니다.

## 한눈에 보기

- 오늘 해야 할 일과 이번 주 흐름을 한 화면에서 확인할 수 있습니다.
- 시간표, 할 일, 목표가 서로 연결된 상태로 작동합니다.
- 집중 화면을 통해 지금 수행해야 하는 항목에 바로 들어갈 수 있습니다.
- Google Calendar / Google Tasks 연동을 통해 기존 도구와 자연스럽게 연결됩니다.
...
- 이벤트, 할 일, 목표, 집중 흐름, 동기화, 제안 기능의 기본 구조
...
- LLM: vLLM 기반 Gemma 4 서빙, OpenAI 호환 API 연동
- Deployment: Docker
...
Gemma 4 추론 서버까지 함께 띄우려면 루트 `.env`에 `HF_TOKEN`을 넣고 아래처럼 실행합니다.
...
├─ gemma4/       Gemma 4 실험 자산과 vLLM 연동 보조 파일
├─ .local/       로컬 전용 문서와 시크릿
└─ README.md
```
...
- 더 믿을 수 있는 동기화 흐름
- 더 실용적인 AI 제안 품질
- 컨테이너 기반 운영 및 배포 흐름 정비

## 한마디로

Time Table은 “계획을 적는 도구”보다 “한 주를 운영하는 화면”에 더 가깝습니다.

캘린더, 태스크, 목표, 집중을 따로 보지 않고 하나의 실행 흐름으로 다루고 싶을 때, 이 프로젝트가 그 기반이 되는 것을 목표로 합니다.
